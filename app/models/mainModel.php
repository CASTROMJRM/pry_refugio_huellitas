...
// Se incluye el server.php que tiene los datos de la conexión a la bd
    ...
    class mainModel{
        ...
        //Nuevo Función de Imagen 11_junio_
        public function seleccionarImgSlider(){
            $sql=$this->conectar()->prepare("SELECT * FROM slider WHERE status=1");
            $sql->execute();
            return $sql;
        }
         //Nueva funcion de card
         public function seleccionarImgCard(){
            $sql=$this->conectar()->prepare("SELECT * FROM tblmascota WHERE status=1");
            $sql->execute();
            return $sql;
        }
